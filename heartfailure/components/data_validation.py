...
from heartfailure.entity.artifact_entity import DataIngestionArtifact,DataValidationArtifact
from heartfailure.entity.config_entity import DataValidationConfig
from heartfailure.exception.exception import heartfailureException
from heartfailure.logging.logger import logging
from heartfailure.constant.training_pipeline import SCHEMA_FILE_PATH
from heartfailure.utils.main_utils.utils import read_yaml_file,write_yaml_file
...
class DataValidation:
    ...
    def validate_numbers_columns(self,dataframe:pd.DataFrame)-> bool:
        try:
    ...
    def validate_numerical_columns_exist(self, dataframe:pd.DataFrame) -> bool:
        try:
            logging.info(f"Checking numerical column exists or not in {len(dataframe.columns)}")
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            status = self.validate_numbers_columns(dataframe=train_dataframe)
            if not status:
                error_message = f" Train dataframe does not contain all columns. "
            
            status = self.validate_numbers_columns(dataframe=test_dataframe)
            if not status:
